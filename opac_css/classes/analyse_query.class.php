...
// +-------------------------------------------------+
// $Id: analyse_query.class.php,v 1.62 2013-05-16 07:35:22 mbertin Exp $
	...
	function get_query_r_mot_with_table_tempo_all($tree,$field_id,$table_mot,$field_mot,$table_term,$field_term,$restrict,$neg_restrict=false,$all_fields=false) {
		...
		for ($i=0; $i<count($tree); $i++) {
			...
			if($tree[$i]->sub){
				...
				if($tree[$i]->literal == 0){
					// on commence...
					$elem_query = "select distinct $field_id from $table_mot join words on num_word = id_word where ";
	...
	function get_pert($notices_ids,$restrict=array(),$neg_restrict=false,$with_explnum=false,$return_query = false,$all_fields=false){
		...
		if(count($notices_ids)){
			...
			if($this->input !== "*"){
				...
				if(count($words)){
					$pert_query_words = "";
					//$pert_query_words.= "select id_notice as notice_id, sum(!!pert!!) as pert from notices_mots_global_index join words on num_word = id_word where ";
					$pert_query_words.= "select id_notice as notice_id, !!pert!! as pert from notices_mots_global_index join words on num_word = id_word where ";
					$where = "";
					foreach($words as $term){
						...
						$pert_query_words = str_replace("!!pert!!","max((".$crit.") * pond *".$tpound.")+!!pert!!",$pert_query_words);
					}
