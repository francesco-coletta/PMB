...
// +-------------------------------------------------+
// $Id: facette_search.class.php,v 1.33.2.2 2013-06-13 09:42:49 mbertin Exp $
...
class facettes{
	...
	function create_table_facettes(){
		...
		<script> 		
			...
			function facette_see_more(id){
				var req = new http_request();
				var url='./ajax.php?module=ajax&categ=facette&sub=see_more&id=' + id;
				req.request(url);
				var textRetour = req.get_text();
				var jsonArray = JSON.parse(textRetour);
			...
			foreach ($vTabFacette['value'] as $keyValue=>$vLibelle) {
				...
				if (!$clicked) {
					...
					if ($affiche) {
						...
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['facette'][]=$vLibelle." "."(".$vTabFacette['nb_result'][$keyValue].")";
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['value'][]=$vLibelle;	
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['nb_result'][]=$vTabFacette['nb_result'][$keyValue];
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['code_champ']=$vTabFacette['code_champ'];
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['code_ss_champ']=$vTabFacette['code_ss_champ'];
						$_SESSION['facette_plus'][$this->tab_facettes_record[$keyFacette]['id']]['name']=$vTabFacette['name'];
					}
				}
			}
		}
		...
		if (count($arrayFacettesNotClicked)) {
			...
			</script><form name='facettes_multi' method='POST' action='./index.php?lvl=more_results&mode=extended&facette_test=1'>";
				...
				foreach ($facette as $detailFacette) {
					...
					if (!isset($detailFacette['see_more'])) {					
						...
						$table_facette .= "
							<tr>
								<td colspan='2'>
									<a href='javascript:facette_see_more(".$idfacette.");'>".$msg["facette_plus_link"]."</a>
								</td>
							</tr>";
						$flagSeeMore = true;
					}
	...
	public static function do_level1() {
		...
		if (($_SESSION["level1"])&&(!$autolevel1)&&($tab!="affiliate")) {
			...
			if (($opac_autolevel2)&&($autolevel1)&&($tab!="affiliate")) {
				//G�n�ration du post et du get... 
				//Attention tous ce qui passe par ajax.php doit �tre en utf-8
				$table="<script>";
				$to_submit="";
	...
	public static function make_facette_suggest($id_notice_array){
		...
		//on recr�e un tableau pour regrouper les �ventuels doublons
		$tmpArray = array();
	...
	function aff_notices_list($notices_list){
		...
		foreach($notices_list as $notice_id){		
			...
			$res = @mysql_query($req,$dbh);
			if($r=mysql_fetch_object($res)){
				$image="<img class='vignetteimg_simili' src='./images/no_image.jpg' hspace='4' vspace='2'>";	
				if ($r->code || $r->thumbnail_url) {
					if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $r->thumbnail_url)) {
						$code_chiffre = pmb_preg_replace('/-|\.| /', '', $r->code);
						$url_image = $opac_url_base."getimage.php?url_image=".urlencode($opac_book_pics_url)."&noticecode=!!noticecode!!&vigurl=".urlencode($r->thumbnail_url);
						...
						$image = "<img class='vignetteimg_simili' src='".$url_image_ok."' title=\"".$title_image_ok."\" hspace='4' vspace='2'>";						
			...
			$img_list.="<td align='center'>".$image."</td>";
			$title_list.="<td align='center'>".$notice_header."</td>";
		}
		if(!$i)return"";
		$display="<table width='100%' style='table-layout:fixed;'><tr>".$img_list."</tr><tr>".$title_list."</tr></table>";
		return $display;
	}
