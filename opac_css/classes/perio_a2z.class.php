...
// +-------------------------------------------------+
// $Id: perio_a2z.class.php,v 1.32.2.3 2013-09-19 12:14:27 dgoron Exp $
			...
			foreach($this->onglets_sub_contens[$onglet_num] as $ongletSub_num => $ongletSub){
				if($ongletSub_num==$ongletSub_sel){
					...
					if (is_array($ongletSub["id"])) {
						...
						$serials_ids = implode(",", $ongletSub["id"]);
						$query = "select notice_id,tit1 from notices where notice_id in(".$serials_ids.") order by tit1";
