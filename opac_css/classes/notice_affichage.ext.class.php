...
// +-------------------------------------------------+
// $Id: notice_affichage.ext.class.php,v 1.182.2.13 2013-09-11 15:10:22 mbertin Exp $
	...
	function genere_double($depliable=1, $premier='ISBD') {
		...
		if ($depliable) {
			$template="
				...
				$case_a_cocher
	    		<img class='img_plus".$this->abiodoc_app_val."' src=\"./getgif.php?nomgif=plus\" name=\"imEx\" id=\"el!!id!!Img\" title=\"".$msg['expandable_notice']."\" border=\"0\" onClick=\"expandBase('el!!id!!', true); return false;\" hspace=\"3\" />";
...
class notice_affichage_chu_angers extends notice_affichage{
	...
	function do_header($id_tpl=0) {
		...
		//Affichage perso pour header notice chu angers abacarisse
		$notice_header_suite = "";
		
		$this->notice_header.='<table>';
		$this->notice_header.='<tr>';
		$this->notice_header.='<td style="width:80%;">';
		
		if($this->notice->niveau_biblio=='a' && $this->notice->niveau_hierar=='2'){
			//Articles
			//Auteurs
			$auteurs='';
			if(sizeof($this->responsabilites['auteurs'])){
				foreach($this->responsabilites['auteurs'] as $auteur){
					...
					if($auteur['rejete']){
						$auteurs.=' '.$auteur['rejete'];
					}
				}
			}
			if($auteurs){
				$this->notice_header.='<strong>'.$auteurs.'</strong>'.' - ';
			}
			//titre
			$this->notice_header.='<strong>'.$this->notice->tit1.'</strong>';
			
			$parent='';
			//Informations parent
			if($this->parent_title){
				$parent.=' - <i style="font-style:italic;">'.$this->parent_title.'</i>';
			}
			if($this->parent_date){
				$parent.=' - '.$this->parent_date;
			}
			if($this->parent_numero){
				$parent.=', '.$this->parent_numero;
			}
			//collation
			if($this->notice->npages){
				$parent.=' - '.$this->notice->npages;
			}
			if(!$this->to_print){
				$this->notice_header.=inslink($parent,'index.php?lvl=bulletin_display&id='.$this->bulletin_id);
			}else{
				$this->notice_header.=$parent;
			}
			...
			//titre
			$this->notice_header.='<strong style="font-style:italic;">'.$this->notice->tit1.'</strong>';
				
			//mention d'édition
			if($this->notice->mention_edition){
				$this->notice_header.=' - '.$this->notice->mention_edition;
			}
			...
			if($this->notice->year){
				...
				$editions.=$this->notice->year;
			}
			if($editions){
				$this->notice_header.=' - '.$editions;
			}
				
			//collation
			if($this->notice->typdoc=='g'){
				if($this->notice->size){
					$this->notice_header.=' - '.$this->notice->size;
				}
			}else{
				if($this->notice->npages){
					$this->notice_header.=' - '.$this->notice->npages;
				}
			}
			...
			if($this->notice->coll_id){
				$collection = new collection($this->notice->coll_id);
				$this->notice_header.=' ('.$collection->name.')';
			}
		}
		
		//Liens
		if($this->notice->lien && !$this->to_print){
			$this->notice_header.=' - <a href="'.$this->notice->lien.'">'.$this->notice->lien.'</a>';
		}
		...
		//mots clés
		if($this->categories_toutes){
			$this->notice_header.='<span style="font-size:11px;">'.$this->categories_toutes.'</span><br/>';
		}else{
			//retour ligne
			$this->notice_header.='<br/>';
		}
		...
		$img='';
		if ($icon && !$this->to_print) {
			$info_bulle_icon=str_replace("!!niveau_biblio!!",$biblio_doc[$this->notice->niveau_biblio],$msg["info_bulle_icon"]);
			$info_bulle_icon=str_replace("!!typdoc!!",$tdoc->table[$this->notice->typdoc],$info_bulle_icon);
			$img.="<img src=\"".$opac_url_base."images/$icon\" alt='".$info_bulle_icon."' title='".$info_bulle_icon."'/>";
		}
		$nl='';
		
		//notes et résumé
		if($this->notice->n_gen){
			$nl.=$this->notice->n_gen.'<br/>';
		}
		if($this->notice->n_contenu){
			$nl.=$this->notice->n_contenu.'<br/>';
		}
		$this->notice_header.='<div style="float:left;margin:10px;margin-top:0px;">'.$img.'</div><i style="font-size:11px;font-style:italic">'.$nl.'</i>';
		$this->notice_header.='</td>';
		//Cote + vignette
		$this->notice_header.='<td>';
		$cote='';
		
		if($this->notice->niveau_biblio=='a' && $this->notice->niveau_hierar=='2'){
			$req='SELECT expl_cote FROM exemplaires WHERE expl_bulletin='.$this->bulletin_id.' LIMIT 1';
		}else{
			$req='SELECT expl_cote FROM exemplaires WHERE expl_notice='.$this->notice_id.' LIMIT 1';
		}
		...
		if(!$this->to_print){
			...
			if ($this->notice->code || $this->notice->thumbnail_url) {
				if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->notice->thumbnail_url)) {
					...
					$title_image_ok = "";
					if(!$this->notice->thumbnail_url) $title_image_ok = htmlentities($opac_book_pics_msg, ENT_QUOTES, $charset);
					if ($depliable) $image = "<div><img style='width:60px;' src='".$opac_url_base."images/vide.png' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2' isbn='".$code_chiffre."' url_image='".$url_image."' vigurl=\"".$this->notice->thumbnail_url."\" /></div>";
					else {
						if ($this->notice->thumbnail_url) {
							$url_image_ok=$this->notice->thumbnail_url;
						} else {
						...
						$image = "<div><img style='width:60px' src='".$url_image_ok."' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2' /><div>";
					}
				} else $image="" ;
				if ($image) {
					$entree = "$image<div style='clear: both;font-weight: bold;padding-top: 10px;text-align: right;'>$cote</div>" ;
				} else {
					$entree = "<div style='clear: both;font-weight: bold;padding-top: 10px;text-align: right;'>$cote</div>" ;
				}
...
// 		$this->notice_public= $this->genere_in_perio ();
		...
		//indexint
		if($this->notice->indexint && !$this->to_print){
			$indexint=new indexint($this->notice->indexint);
		...
		$this->p_perso->get_values($this->notice_id) ;
		if($this->p_perso->values[4][0]){
			$this->notice_public.="<tr><td align='right' class='bg-grey'><span class='etiq_champ'>$msg[demandes_theme] :</span></td><td>".$this->p_perso->get_formatted_output($this->p_perso->values[4],4)."</td></tr>";
		}
		//prix
		if($this->notice->prix){
...
class notice_affichage_cconstitutionnel extends notice_affichage {
	...
	function do_header($id_tpl=0) {
		...
		if(($this->notice->typdoc=='a') || ($this->notice->typdoc=='j') 
			...
			if ($res) {
				if(mysql_num_rows($res)){
					$tmp_notice_header .= "<br />".$msg['cote_start']." ".mysql_result($res,0,0);	
				}
			}
			if ($this->notice->statut == 3) {
...
class notice_affichage_ctles extends notice_affichage {
	...
	function do_public($short=0,$ex=1) {
		...
		// zone de l'éditeur 
		if ($this->notice->year)
			$annee = "<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".$msg['year_start']."</span></td><td>".$this->notice->year."</td></tr>" ;
		...
		// ISBN ou NO. commercial
		if ($this->notice->code) $this->notice_public .= "<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".$msg['code_start']."</span></td><td>".$this->notice->code."</td></tr>";
	...
	function aff_suite() {
		...
		$ret_index = "";
		// Catégories
		if ($this->categories_toutes) $ret_index .= "<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".$msg['categories_start']."</span></td><td>".$this->categories_toutes."</td></tr>";
		...
		if (!$this->p_perso->no_special_fields) {
			...
			for ($i=0; $i<count($this->memo_perso_["FIELDS"]); $i++) {
				$p=$this->memo_perso_["FIELDS"][$i];
				if ($p['OPAC_SHOW'] && $p["AFF"]) $perso_aff .="<tr><td align='left' class='bg-grey'><span class='etiq_champ'>".strip_tags($p["TITRE"])."</span></td><td>".$p["AFF"]."</td></tr>";
			}
		}
		$ret .= $perso_aff ;
		...
		if(($this->coll_state_list_nbr = mysql_num_rows($myQuery))) {
			...
			while(($coll = mysql_fetch_object($myQuery))) {
				...
				if($empr_location==$my_collstate->location_id){
					// modif des notes
					$tpl_note_modif="<script src='".$include_path."/javascript/coll_modif.js'></script>
