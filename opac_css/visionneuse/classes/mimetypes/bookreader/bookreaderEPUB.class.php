...
// +-------------------------------------------------+
// $Id: bookreaderEPUB.class.php,v 1.5 2013-05-07 15:03:02 apetithomme Exp $
...
class bookreaderEPUB {
	...
	function bookreaderEPUB($doc,$parameters){
    	...
    	$this->ebook = new epubData($visionneuse_path."/temp/".$this->doc->id);
	...
	function getPage($page){
		...
		if (!file_exists($visionneuse_path."/temp/page_".$this->doc->id."-".str_pad($page, $len,"0",STR_PAD_LEFT).".".$extension)) {
			...
			if ($format == "imagick") {
				exec("pdftoppm -f $page -l $page -r ".$resolution." ".$visionneuse_path."/temp/".$this->doc->id.".pdf ".$visionneuse_path."/temp/page_".$this->doc->id);
				$imagick = new Imagick();
	...
	function search($user_query){
		...
		if (!file_exists($visionneuse_path."/temp/".$this->doc->id.".bbox")){
			exec("pdftotext -bbox ".$visionneuse_path."/temp/".$this->doc->id.".pdf ".$visionneuse_path."/temp/".$this->doc->id.".bbox");
			//bbox ne gère pas les entités html présentent dans le titre
			$contents = file_get_contents($visionneuse_path."/temp/".$this->doc->id.".bbox");
			if ((preg_match("/\<title\>(.*)\<\/title\>/", $contents, $match)) && ($match[1])) {
				file_put_contents($visionneuse_path."/temp/".$this->doc->id.".bbox", str_replace($match[1], htmlentities($match[1], ENT_QUOTES, $charset), $contents));
			}
		}
	...
	function getBookmarks(){
		...
		$bookmarks = array();
		$toc = new DOMDocument();
		$toc->load($visionneuse_path."/temp/".$this->doc->id."_toc.xml");
	...
	function generatePDF(){
		...
		if (!file_exists($visionneuse_path."/temp/".$this->doc->id.".pdf")){
			$zip = new ZipArchive();
			$res = $zip->open($visionneuse_path."/temp/".$this->doc->id);
			
			if ($res === true) {
				if (!is_dir($visionneuse_path."/temp/".$this->doc->id."_unzip")) mkdir($visionneuse_path."/temp/".$this->doc->id."_unzip");
				$zip->extractTo($visionneuse_path."/temp/".$this->doc->id."_unzip");
				$zip->close();
				$tab_html_docs = array();
				...
				foreach ($items as $file) {
					if ($file["media-type"] == "text/css") {
						$file_path = $visionneuse_path."/temp/".$this->doc->id."_unzip/".$opfdir.$file["href"];
				...
				foreach ($this->getHtmlOrdered() as $file) {
					$file_path = $visionneuse_path."/temp/".$this->doc->id."_unzip/".$file;
				...
				exec("wkhtmltopdf --title ".str_replace(" ", "\ ", $titre)." --dump-outline ".$visionneuse_path."/temp/".$this->doc->id."_toc.xml --footer-center [page] cover ".$list_html_docs." ".$visionneuse_path."/temp/".$this->doc->id.".pdf");
				
				$this->rrmdir($visionneuse_path."/temp/".$this->doc->id."_unzip");
			} else {
				print "Erreur à l'ouverture de l'ebook!";
			}
		}
		return $visionneuse_path."/temp/".$this->doc->id.".pdf";
	}
