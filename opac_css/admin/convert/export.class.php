...
// +-------------------------------------------------+
// $Id: export.class.php,v 1.26 2013-04-17 08:37:34 mbertin Exp $
	...
	function get_next_notice($lender = "", $td = array(), $sd = array(), $keep_expl = false, $params=array(), $force_diffusion=false) {
		...
		if ($this -> current_notice != -1) {
			...
			if (($ed1 = mysql_fetch_object($resultat))) {
				...
				$subfields["b"] = trim($ed1 -> ed_adr1.' '.$ed1 -> ed_adr2.' '.$ed1 -> ed_cp.' '.$ed1 -> ed_ville.' '.$ed1 -> ed_pays);
			...
			if ($keep_expl) {
				if($res->niveau_biblio == 'b' && $res->niveau_hierar == '2'){//Si c'est une notice de bulletin
					...
					$requete = "select expl_id, create_date, expl_cb,expl_cote,expl_statut,statut_libelle, statusdoc_codage_import, expl_typdoc, tdoc_libelle, tdoc_codage_import, expl_note, expl_comment, expl_section, section_libelle, sdoc_codage_import, expl_owner, lender_libelle, codestat_libelle, statisdoc_codage_import, expl_date_retour, expl_date_depot, expl_note, pret_flag, location_libelle, locdoc_codage_import from exemplaires, docs_statut, docs_type, docs_section, docs_codestat, lenders, docs_location where expl_notice=".$res -> notice_id." and expl_statut=idstatut and expl_typdoc=idtyp_doc and expl_section=idsection and expl_owner=idlender and expl_codestat=idcode and expl_location=idlocation";
					if (($lender != "x")&&($lender!=""))
			...
			//Descripteurs
			$requete="SELECT libelle_categorie,categories.num_noeud,categories.langue FROM categories, notices_categories WHERE notcateg_notice=".$res->notice_id." and categories.num_noeud = notices_categories.num_noeud ORDER BY ordre_categorie";
            $resultat=mysql_query($requete);
