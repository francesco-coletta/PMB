...
// +-------------------------------------------------+
// $Id: notice_info.class.php,v 1.38.2.2 2013-08-26 07:25:11 mbertin Exp $
	...
	function fetch_data() {
		...
		if ($res->niveau_biblio=='b') {
			...
			if(($bull=mysql_fetch_object($result_bull))){				
				...
				$this->bulletin_date_date=$bull->date_date;
		...
		if ($this->notice->code || $this->notice->thumbnail_url) {
			if ($opac_show_book_pics=='1' && ($opac_book_pics_url || $this->notice->thumbnail_url)) {
				...
				$this->memo_image = "<img class='vignetteimg' src='".$url_image_ok."' title=\"".$title_image_ok."\" align='right' hspace='4' vspace='2'>";
