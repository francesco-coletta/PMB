...
// +-------------------------------------------------+
// $Id: category.class.php,v 1.35 2013-04-11 08:06:40 mbertin Exp $
...
function has_notices() {
	...
	function get_informations_from_unimarc($fields,$link = false){
		$data = array();
		
		if(!$link){
			$data['label'] = $fields['250'][0]['a'][0];
			if($fields['250'][0]['j']){
				for($i=0 ; $i<count($fields['250'][0]['j']) ; $i++){
					$data['label'] .=  " -- ".$fields['250'][0]['j'][$i];
				}
			}
			if($fields['250'][0]['x']){
				for($i=0 ; $i<count($fields['250'][0]['x']) ; $i++){
					$data['label'] .=  " -- ".$fields['250'][0]['x'][$i];
				}
			}
			if($fields['250'][0]['y']){
				for($i=0 ; $i<count($fields['250'][0]['y']) ; $i++){
					$data['label'] .=  " -- ".$fields['250'][0]['y'][$i];
				}
			}
			if($fields['250'][0]['z']){
				for($i=0 ; $i<count($fields['250'][0]['z']) ; $i++){
					$data['label'] .=  " -- ".$fields['250'][0]['z'][$i];
				}
			}		
	...
	function update($data,$id_thesaurus,$num_parent,$lang){
		...
		if($this->id == 0){
			...
			if(mysql_num_rows($result)){
				...
				$id = $n->id_noeud;
				$c=new categories($id, $lang);
				$c->libelle_categorie= addslashes($data['label']);
				$c->note_application = addslashes($data['note']);
				$c->comment_public = addslashes($data['comment']);
				$c->save();
				$this->id = $c->num_noeud;
			}
		}else{
