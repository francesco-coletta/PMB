...
// +-------------------------------------------------+
// $Id: notice.class.php,v 1.156.2.2 2013-09-19 08:16:54 dgoron Exp $
...
// 				if ($this->notice_mere_id || $this->duplicate_from_id) $link_annul = "<input type='button' class='bouton' value=\"{$msg[76]}\" onClick=\"unload_off();history.go(-1);\" />";
				...
				foreach($temp_ext as $k=>$v) {
					...
					for ($i = 0; $i<count($v["TABLE"]); $i++) {
						...
						if($isbd_ask_list[$k]){ // isbd  => memo de la requete pour retrouver les id des autorités							
							...
							$req="select $id_aut as id_aut_for_isbd from notices".$jointure." where ".$temp_not['t'][0][0].".".$tableau['REFERENCEKEY'][0][value]."=".$notice;	
							$isbd_tab_req[]=$req;															
						}
		...
		static function majNotices($notice){
			global $pmb_keyword_sep;
			if($notice){
				$query = mysql_query("SELECT notice_id,tparent_id,tit1,tit2,tit3,tit4,index_l, n_gen, n_contenu, n_resume, tnvol FROM notices WHERE notice_id='".$notice."'");
