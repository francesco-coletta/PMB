...
// +-------------------------------------------------+
// $Id: alter_v5.inc.php,v 1.263 2013-05-24 13:52:34 touraine37 Exp $
...
/*
	...
	A mettre en 5.07
		...
		// en attente ER
		ALTER TABLE explnum ADD explnum_vignette_mimetype VARCHAR(255) NOT NULL default '' AFTER explnum_vignette
		update explnum set explnum_vignette_mimetype=explnum_mimetype where explnum_mimetype != ''

		//ajout d'un champ section_comment dans la table docs_section // abandonné pour l'instant ER/DG 17/11/2011
		//$rqt = "ALTER TABLE docs_section ADD section_comment TEXT ";
		//echo traite_rqt($rqt,"ALTER TABLE docs_section ADD section_comment ");
		
		// A METTTRE AILLEURS 
  		require_once($class_path."/double_metaphone.class.php");
		require_once($class_path."/stemming.class.php");
		$rqt = "select id_word,word from words";
		$res = mysql_query($rqt);
		if(mysql_num_rows($res)){
			while($row = mysql_fetch_object($res)){
