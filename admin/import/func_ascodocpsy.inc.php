...
// +-------------------------------------------------+
// $Id: func_ascodocpsy.inc.php,v 1.2.2.1 2013-09-12 09:18:06 dgoron Exp $
...
function import_new_notice_suite() {
	...
	if (count($info_903)) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_prodfich'"),0);
	...
	//DIPSPE
	if ($info_904[0]) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_dipspe'"),0);
	...
	//Annexe
	if (count($info_905)) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_annexe'"),0);
	...
	//Lien annexe
	if (count($info_906)) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_lienanne'"),0);
	...
	//Localisation
	if (count($info_907)) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_loc'"),0);
		$requete="select max(notices_custom_list_value*1) from notices_custom_lists where notices_custom_champ=".$cp_id;
		$resultat=mysql_query($requete);
		$max=@mysql_result($resultat,0,0);
		$n=$max+1;
	...
	//Nature du texte
	if (count($info_908[0])) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_nattext'"),0);
	...
	//Date du texte
	if (count($info_909[0])) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_datetext'"),0);
	...
	if (count($info_910[0])) {
		$cp_id = mysql_result(mysql_query("select idchamp from notices_custom where name='cp_numtexof'"),0);
...
function do_thesaurus_ascodocpsy($id_thesaurus, $nom_categ, $branch_values=array(), $lang='fr_FR', $ordre_categ=0) {
	...
	$parent_thes = mysql_result(mysql_query("select id_noeud from noeuds where autorite='TOP' and num_thesaurus='".$id_thesaurus."'"),0,0);
...
function genere_bulletin($perio_info,$bull_info,$isbull=true){
	...
	$search = "select bulletin_id from bulletins where bulletin_numero LIKE '".$bull_info['num']."' and bulletin_notice = $perio_id";
	$res = mysql_query($search);
	if(mysql_num_rows($res) == 0){
