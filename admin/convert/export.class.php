...
// +-------------------------------------------------+
// $Id: export.class.php,v 1.65.2.2 2013-06-26 08:41:38 dgoron Exp $
...
class export {
	...
	function get_next_bulletin($lender = "", $td = array(), $sd = array(), $keep_expl = false, $params=array()){
		...
		if(mysql_num_rows($res)){
			...
			$resultat = mysql_query($requete);
			while (($ex = mysql_fetch_object($resultat))) {
				if ( (in_array($ex->expl_id,$params['export_only_expl_ids']))  ||  (!is_array($params['export_only_expl_ids']))  ||  (!count($params['export_only_expl_ids'])) ) {
					$subfields = array();
					global $export996 ;
					$export996 = array() ;
					$subfields = export_traite_exemplaires ($ex);
