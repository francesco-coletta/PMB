...
// +-------------------------------------------------+
// $Id: avis_notice.inc.php,v 1.6 2011-10-07 10:20:19 ngantier Exp $
...
function avis_notice($id,$avis_quoifaire,$valid_id_avis){
	...
	global $base_path;
	global $pmb_javascript_office_editor,$opac_avis_note_display_mode;
	...
	$nb_avis=mysql_numrows($r);
		...
		while ($loc = mysql_fetch_object($r)) {
			...
			if($opac_avis_note_display_mode==2){
				$note=$msg["avis_detail_note_".$loc->note]." ";
			}else if($opac_avis_note_display_mode!=0){
				$note=$msg[gestion_avis_note]." ".$loc->note." ";
			}else $note="";
